...
class OrderImportDetail extends Mapper implements \MVC\Domain\OrderImportDetailFinder {
    ...
    function __construct() {
		...
		$tblResource = "cafepassion_resource";
		$tblOrderImport = "cafepassion_order_import";
		$tblOrderImportDetail = "cafepassion_order_import_detail";
		$tblSessionDetail = "cafepassion_session_detail";
		$tblR2C = "cafepassion_r2c";
		...
		$trackByExportStmt = sprintf("
			...
			from
				cafepassion_session S inner join cafepassion_session_detail SD on S.id = SD.idsession
			where
				SD.idcourse IN(select id_course from cafepassion_r2c where id_resource=?) AND
