...
class App extends Mapper implements \MVC\Domain\AppFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from cafethemxua_app");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from cafethemxua_app where id=?");
		$this->findByStmt = self::$PDO->prepare( 
                            "select * from cafethemxua_app where alias=?");
		
		$this->notSignedStmt = self::$PDO->prepare("
							SELECT * 
							FROM `cafethemxua_app` 
							WHERE 
								id NOT IN ( 
        ...
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into cafethemxua_app ( name, phone, address, email, banner, prefix, app, datecreate, dateupdate, dateactivity, type) values(?,?,?,?,?,?,?,?,?,?,?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from cafethemxua_app where id=?");
		$this->checkStmt = self::$PDO->prepare( 
                            "select distinct id from cafethemxua_app where app=?");			
		$this->checkEmailStmt = self::$PDO->prepare( 
                            "select distinct id from cafethemxua_app where email=?");									
