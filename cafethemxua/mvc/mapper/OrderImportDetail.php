...
class OrderImportDetail extends Mapper implements \MVC\Domain\OrderImportDetailFinder {
    ...
    function __construct() {
		...
		$tblResource = "cafethemxua_resource";
		$tblOrderImport = "cafethemxua_order_import";
		$tblOrderImportDetail = "cafethemxua_order_import_detail";
		$tblSessionDetail = "cafethemxua_session_detail";
		$tblR2C = "cafethemxua_r2c";
		...
		$trackByExportStmt = sprintf("
			...
			from
				cafethemxua_session S inner join cafethemxua_session_detail SD on S.id = SD.idsession
			where
				SD.idcourse IN(select id_course from cafethemxua_r2c where id_resource=?) AND
